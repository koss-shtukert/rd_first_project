...
class Hw13TasksScreen extends StatefulWidget {
  const Hw13TasksScreen({super.key});

  @override
  State<Hw13TasksScreen> createState() =>
      _WidgetConstrainsTrainingScreenState();
}

class _WidgetConstrainsTrainingScreenState extends State<Hw13TasksScreen> {
  int _currentIndex = 0;
...
class TrainingExample12 extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const redContainerWidth = 500.0;

    return LayoutBuilder(
      builder: (context, constraints) {
        final maxWidth = constraints.maxWidth;
        final redWidth = redContainerWidth.clamp(0.0, maxWidth);
        final greenWidth = (maxWidth - redWidth).clamp(0.0, maxWidth);
        ...
        return Row(
          children: [
            ...
            if (greenWidth > 0)
              Container(color: Colors.green, height: 100, width: greenWidth),
          ],
        );
      },
    );
...
// передає tight constraints (width: 100, height: 100), які мають пріоритет.
// ConstrainedBox може лише звужувати constraints від батька,
// але не розширювати їх.
...
/// Відповідь: Column розміщує дітей в порядку оголошення, але малює їх у зворотньому
/// порядку. Зелений контейнер через OverflowBox виходить за межі
/// свого SizedBox (100px) і накладається на сусідів.
/// Але, оскільки відмальовка йде зверху вниз по списку:
/// 1) Червоний малюється першим
/// 2) Зелений (OverflowBox) малюється другим — поверх червоного
/// 3) Синій малюється третім — поверх зеленого
/// Тому зелений перекриває червоний, але синій перекриває зелений.
...
// Відповідь: LimitedBox діє лише коли батько не накладає обмежень.
// У першому Column він працює, бо Column дає дітям unbounded height.
// У другому випадку LimitedBox обгорнутий у SizedBox(height: 100),
// який передає tight constraint. Оскільки батько вже задав конкретну висоту,
// LimitedBox ігнорує свій maxHeight і зелений контейнер займає всі 100px.
...
// Відповідь: LimitedBox діє лише коли батько
// дає unbounded constraints. Оскільки constraints вже обмежені,
// maxHeight: 80 ігнорується і ColoredBox займає стільки місця,
// скільки потрібно для відображення тексту.
